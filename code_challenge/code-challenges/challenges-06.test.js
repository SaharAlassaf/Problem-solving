...
const houseSize = (arr) => {
  ...
  arr.forEach((element) => {
    ...
    if (element.spouse) total += 1;
    total += element.children.length;
    sizes.push({ house: element.house, members: total });
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach((element) => {
    let total = 0;
    if (element.name) total += 1;
    if (element.spouse && !deceasedSpouses.includes(element.spouse)) total += 1;
    total += element.children.length;
    survivors.push({ house: element.house, members: total });
  });
  return survivors;
};
